...
const dataPath = __dirname + "/Atta/approvedThreads.json";
const dataPending = __dirname + "/Atta/pendingdThreads.json";
const fs = require("fs");
...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
      ...
      api.sendMessage(`[ OK ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`, idBox);
   	...
   	else api.sendMessage("[ OK ] ✨ApKa Group Approved Kar Diya Hai🙌.\n 🖤So Enjoy\n\n 💝🥀𝐎𝐖𝐍𝐄𝐑:- ☞𝕬𝖙𝖙𝖆𝖚𝖑𝖑𝖆𝖍 𝕶𝖍𝖚𝖍𝖆𝖗𝖔☜ 💫\n 🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 〠𝔸𝕥𝕥𝕒𝕦𝕝𝕝𝕒𝕙🖤\n😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞ www.facebook.com/profile.php?id=attaullah.sindhi.com639\n👋For Any Kind Of Help Contact On Telegram  Username 👉 @Attaullah.khan😇", idBox, (error, info) => {
   		api.changeNickname(` 〖 ${global.config.PREFIX} 〗 ➺ ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, idBox, global.data.botID);
      const axios = require('axios');
	const request = require('request');
	const fs = require("fs");
   let admID = "61579871318829";    
